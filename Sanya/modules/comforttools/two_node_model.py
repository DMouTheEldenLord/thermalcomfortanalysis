...
def cal_ce(ta=None, tr=None, v=None, rh=None, met=None, clo=None, top=None, b=101.325, work=0):
    set_0 = cal_set(ta=ta, tr=tr, v=v, rh=rh, met=met, clo=clo, top=top, b=b, work=work)
    dt = 1e-4
    dx = 100
    ce = 0
    while abs(dx) > 1e-2:
        err1 = set_0 - cal_set(ta=ta - ce, tr=tr - ce, v=0.1, rh=rh, met=met, clo=clo, top=top, b=b, work=work)
        err2 = set_0 - cal_set(ta=ta - (ce + dt), tr=tr - (ce + dt), v=0.1, rh=rh, met=met, clo=clo, top=top, b=b,
                               work=work)
        dx = err1 / (err2 - err1) * dt
        ce = ce - dx
    return ce
